describe('Basic', () => {
  ...
  it('can up arrow to select last item', () => {
    cy.getInStoryByTestId('basic-input')
      .type('{downarrow}') // open menu, last option is focused
      .should('have.value', 'Purple')
  })

  it('can up arrow down select first item', () => {
    cy.getInStoryByTestId('basic-input')
      .type('{downarrow}') // open menu, first option is focused
      .should('have.value', 'Black')
  })

  it('can down arrow to select an item', () => {
    cy.getInStoryByTestId('basic-input')
      .type('{downarrow}') // open menu, firs option is focused
      .type('{downarrow}{enter}') // select second item
      .should('have.value', 'Red')
  })

  it('resets the item on blur', () => {
    cy.getInStoryByTestId('basic-input')
      .type('{downarrow}{enter}{backspace}') // select first item
      .should('have.value', 'Black')
      .getInStory('body')
      .click()
