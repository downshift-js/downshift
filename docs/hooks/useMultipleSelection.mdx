...
| `getItemProps`     | `<span>`                | Called with `index` and `item`, adds tabindex and event listeners.              |
...
equivalent, with the addition of selected items. The styling is completely up to
the developer, as these elements can be `<span>` or `<div>` or anything else,
with a `inline-block` or `flex` display. In the example below, each item has an
`X` icon attached to it, with a `removeItem` callback in the `onClick` handler.
Items are added using `addItem` callback insite the `onStateChange` handler of
`useCombobox`.
