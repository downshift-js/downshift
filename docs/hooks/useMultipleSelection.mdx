...
`X` icon attached to it, with a `removeSelectedItem` callback in the `onClick` handler.
Items are added using `addSelectedItem` callback insite the `onStateChange` handler of
`useCombobox`.
