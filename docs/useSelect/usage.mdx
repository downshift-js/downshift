...
In the example below, we use the `useSelect` hook and destructure from its
...
To build the UI with component libraries such as MaterialUI for example, go to
[UI Libraries](/hooks/use-select/ui-libraries).
...
Codesandbox equivalent is at
[here](https://codesandbox.io/s/useselect-usage-53qfj).
